 ...
 *   }).setLngLat([30.5, 50.5])
 *   .addTo(map);
 * @see [Add custom icons with Markers](https://u-n-l.github.io/unl-map-js-docs/example/custom-marker-icons/)
...
export default class Marker extends Evented {
  ...
  _onMapClick(e: MapMouseEvent) {
    ...
    if (
      this._popup &&
      (targetElement === element || element.contains(targetElement as any))
    ) {
      this.togglePopup();
    }
  }
  ...
  setPitchAlignment(alignment: string) {
    this._pitchAlignment =
      alignment && alignment !== "auto" ? alignment : this._rotationAlignment;
    this._update();
    return this;
  }
